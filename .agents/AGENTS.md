...
- **立足全局，拒绝单次应付（打磨产品 vs 应付式编程铁律）**：
  ...
  - **坚决摒弃“应付式编程”反模式**：严禁只盯眼前单个 Case 搞局部硬套（如看到电饭煲就把背景写死为厨房、看到红色就硬替换为香槟金）。这种投机取巧的硬编码表面上单次似乎变好了，实则给系统埋下了致命暗坑（换个品类立刻穿帮），破坏了模块边界与契约完整性；
  - **坚定践行“打磨产品”思维**：每一个方案与改动，必须从**底层原理、全品类自适应、用户意图忠实度、代码干净度与可扩展性**出发。宁可下慢功夫做系统级抽象与深层机制优化，杜绝为了“凑出单次效果”写无效意淫兜底，也绝不用任何反模式硬编码糊弄过去。
- **保持代码优雅、干净、模块化**：
...
- **字段不确定时严禁靠猜与丑陋多重兜底（必须确定性验证或向用户确认）**：
  - 遇到接口字段名、参数结构或数据类型不确定时，**严禁凭空盲猜，严禁写诸如 `item.data || item.payload || item.info || item.result || ''` 等丑陋意淫的多重兜底代码**；
  - **必须第一时间通过查阅后端源码（如 `admin-claw/server.js`、`../scrm/`）、抓包/接口文档排查确定**；若依然存在歧义或不确定，**必须在第一时间主动向用户提问求证由用户决策**，字段契约必须是百分之百确定的。
- **严禁写毫无意义的自欺欺人兜底代码**：
  - 在“完全由后端动态下发”或“具备明确业务契约”的场景下（如动态工具下发、动态配置读取等），**严禁在前端或插件层写死默认值（如 `tool.name || 'retrieve_fmp'`、`label: 'Second Brain Retrieval'` 等）**。若后端未下发有效数据，必须直接拦截报错或跳过注册，绝不允许使用自己意淫的假默认值强行往下走；
  - **严禁主观假定具体工具行为**：动态工具类型是开放且多样的，系统提示词或 Reminder 中**严禁写死具体的工具名称或狭隘地假定其为某种固定工具（如“一定叫检索工具”）**，必须采用通用的工具驱动描述；
  - **严禁写毫无作用的防御代码**：如在根本无法动态注销已注册工具的底层机制下，在前端写 `registerTools([])` 等自嗨式清空代码；
- **重构即彻底替换，严禁残留脑残历史兼容**：
...
- **核心定位与受众意识（商业客户第一，彻底摒弃极客自嗨思维）**：
  ...
  - 真实客户完全没有、也绝对不应该有任何关于内部底层架构、框架代号（如 `modern_012`）、Prompt 工程、模型参数等技术认知。任何需要用户具备技术心智、记住特定代码/ID 才能跑出好效果的设计，都是**彻底失败的产品设计**。
- **严禁向用户转嫁技术认知与甩锅复杂度（Never Offload Complexity to End Users）**：
  - 严禁向用户提出“请在提示词里指定内部框架编号”、“请提醒 AI 不要偷懒省略 Prompt 细节”、“请手动补充英文术语或光影参数”等极客式的应对建议；
  - 遇到效果欠佳、大模型偷懒或产物单调时，必须第一时间在**产品架构、Prompt 工程约束、技能调度算法或底层网关**中从根源解决，把所有实现复杂度在系统内部彻底消化，绝不允许向用户甩锅。
- **极端偷懒输入下的超预期交付（Zero-Effort Input, Commercial-Grade Output）**：
  - 用户只需输入最简短的大白话（如“我要一张九阳电饭煲海报”），系统和技能就必须具备自动解析、智能丰满、补全爆款版式、材质光影与商业级细节的能力；
  - 必须保证用户心智负担降至最低（Zero-Friction），即使在用户完全不提供专业描述的场景下，输出的结果也必须是开箱即用、质感拉满、符合爆款商业规范的成熟作品。

### 2.16 严禁针对单次 Case 搞硬编码与篡改用户意图准则 (Universal Generalization & Zero-Hardcoding Rule)
- **【严禁针对单个商品或 Case 搞局部硬替换】**：
  - 严禁在系统公共层、网关或拦截器中写死诸如“将商品/背景强行替换为厨房岛台/原木台面”等狭隘的写死逻辑；
  - 系统公共层必须保持 100% 通用自适应与全品类兼容，绝不能因为针对厨电优化而导致数码、美妆等其他品类出现“手机/香水放到厨房岛台”等穿帮滑稽破绽；
- **【严禁篡改用户核心业务意图（如指定颜色、主题、文案）】**：
  - 用户的配色、主题与文案是第一优先级的业务输入，系统底层的职责是**“在百分之百忠实于用户意图的前提下，提升商业摄影光影、材质真实度与排版防截断安全边距”**；
  - 绝对禁止使用正则或粗暴替换强行把用户的纯红/指定色篡改为香槟金等开发者私货偏好；提升高级感必须通过光学景深、物理质感与负向抑制完成，严禁违背用户意志。
...
- **【严禁在缺陷路径上缝缝补补，首选直接规范方案 (Prefer Standard Industrial Solutions)】**：
  - 遇到报错或瓶颈时，严禁顺着局部缺陷的烂路一路打补丁（如明知临时钥匙串有密码和搜索列表缺陷，仍反复在上面套脚本、加变量、试拼写）；
  - **必须第一时间后退一步审视全局**：业界成熟商业项目在此类场景的标准规范解法是什么？（如 macOS CI 标准实践即直接导入系统 `login.keychain-db` 并显式指定证书身份；签名服务即解耦为纯 HTTP 盖章服务）。优先采用经过全球海量项目验证的、最简洁直接的工业级规范解法，做到**一次做对、一次闭环**。
- **【根因确定与验证前绝不轻举妄动】**：
