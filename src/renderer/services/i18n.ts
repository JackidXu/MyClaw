...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    dshSettingsTitle: 'DeepSeek Harness 编程',
    dshExperimentalBadge: '实验',
    dshSettingsDesc: '内置 DeepSeek 官方开源 agent 框架（dsh），提供独立的编程工作台，并允许主 Agent 委托编码任务。启用后，你配置的模型供应商会自动同步给 dsh。',
    ...
    dshSharedDataNote: '直接使用本机的 dsh 目录 ~/.dsh，因此会话历史、工作区、已安装插件、凭证与设置都与你单独运行的 dsh 完全一致，不禁用任何官方能力。LobsterAI 只会往其中追加以「LobsterAI ·」开头的模型供应商条目。',
  ...
  en: {
    ...
    dshSettingsTitle: 'DeepSeek Harness Coding',
    dshExperimentalBadge: 'Experimental',
    dshSettingsDesc: 'Bundles DeepSeek\'s open-source agent harness (dsh) with a standalone coding workbench, and lets the main agent delegate coding tasks to it. Your configured model providers sync into dsh automatically once enabled.',
    ...
    dshSharedDataNote: 'Runs against this machine\'s dsh home (~/.dsh), so history, workspaces, installed plugins, credentials, and settings are exactly what a standalone dsh sees — nothing shipped is disabled. LobsterAI only appends model providers prefixed "LobsterAI ·".',
