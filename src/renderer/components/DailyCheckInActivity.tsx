...
const useDailyCheckInEntryController = ({
  ...
  const isVisible = Boolean(
    enabled && !suppressed && (stateAllowsEntry || success),
  );
  ...
  useEffect(() => {
    ...
    reportDailyCheckInAction(analyticsContext, {
      actionType: DailyCheckInAnalyticsActionType.EntryImpression,
    });
  }, [isLoggedIn, isVisible, snapshot, source]);
    ...
    <>
      <div className="relative">
        <button
          ...
          className="flex h-8 max-w-[176px] items-center gap-1.5 rounded-full border border-[#F1D3C0] bg-[#FFF8F3] px-2.5 text-xs font-medium text-[#7C4328] shadow-subtle transition-colors hover:bg-[#FFF0E6] disabled:cursor-not-allowed disabled:opacity-70 dark:border-[#704530] dark:bg-[#352A25] dark:text-[#F5C4A5] dark:hover:bg-[#403029]"
        >
          {success ? (
