...
if (!gotTheLock) {
  ...
  ipcMain.handle('store:set', async (_event, key, value) => {
    ...
    if (key === 'app_config') {
      ...
      if (syncResult?.success && browserWebAccessChanged && !proxyChanged && actionDecision.impact !== OpenClawConfigImpact.Restart) {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        try {
          ...
          console.warn('[MediaGeneration] OneAPI generate request failed:', e);
        }
      ...
      if (isAsyncTask) {
        ...
        // 注册到后台轮询
        if (sessionId && requestAccountScope) {
