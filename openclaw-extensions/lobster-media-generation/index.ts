...
const VideoGenerateSchema = Type.Object({
  ...
  prompt: Type.Optional(Type.String({ description: 'Text prompt describing the video to generate. Chinese and English supported. For Seedance (2.5/2.0), use explicit @1, @2 tokens in prompt to bind/lock character and product references corresponding to the order of images passed in images/referenceImages.' })),
  ...
  images: Type.Optional(Type.Union([Type.Array(Type.String()), Type.String()], { description: 'Multiple reference image absolute file paths, URLs, or data URLs (e.g. for Seedance character/product locking or multi-reference video generation). If a media reference mapping is provided, use mapped paths; do not pass @ media tokens.' })),
  ...
  durationSeconds: Type.Optional(Type.Union([Type.Number(), Type.String()], { description: 'Video duration in seconds. Valid range depends on model (e.g. Seedance 2.5: up to 30s, Seedance 2.0: 4-15s, MiniMax Hailuo: 6 or 10s). Use -1 for auto. Use action="list" to check.' })),
    ...
    api.registerTool((ctx: any) => {
      ...
      return {
        ...
        description: [
          ...
          'For Seedance models (e.g. Doubao-Seedance-2.5, Doubao-Seedance-2.0), multiple reference images are supported for locking characters and products. Pass all referenced local image paths in the "images" or "referenceImages" array, and use explicit @1, @2 tokens in the "prompt" text corresponding to image indices (e.g., "@1 is the character, @2 is the product").',
