...
- Register each successful asset immediately. Do not start the next generation until `heyclaw_skin_manage` confirms registration.
...
- Never write skin files, application configuration, CSS, or databases directly. Only `heyclaw_skin_manage` may register or apply a skin.
...
- LobsterAI route: call `heyclaw_image_generate` with `action="list"`, choose one available image model when no model is already fixed, then keep that model.
...
- for `heyclaw_image_generate`, call `action="status"` once with the task ID; the tool owns adaptive polling;
...
Call `heyclaw_skin_manage` with `action="status"` and the draft ID. Apply only when both required slots are registered and ready.
