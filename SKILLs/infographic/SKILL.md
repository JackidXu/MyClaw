---
name: infographic
description: Generate professional infographics with 21 layout types and 22 visual styles. Analyzes content, recommends layout×style combinations, and generates publication-ready infographics. Use when user asks to create "infographic", "信息图", "visual summary", "可视化图", "图文解说图", "结构化图" or "数据图".
version: 1.117.4
metadata:
  openclaw:
...
| Layout | Best For |
|--------|----------|
| `linear-progression / 线性流程` | Timelines, processes, tutorials |
| `binary-comparison / 二元对比` | A vs B, before-after, pros-cons |
| `comparison-matrix / 多维对比矩阵` | Multi-factor comparisons |
| `hierarchical-layers / 层级金字塔` | Pyramids, priority levels |
| `tree-branching / 树形分支` | Categories, taxonomies |
| `hub-spoke / 中心辐射` | Central concept with related items |
| `structural-breakdown / 结构分解` | Exploded views, cross-sections |
| `bento-grid / 便当网格` | Multiple topics, overview (default) |
| `iceberg / 冰山模型` | Surface vs hidden aspects |
| `bridge / 桥接方案` | Problem-solution |
| `funnel / 漏斗转化` | Conversion, filtering |
| `isometric-map / 等距空间` | Spatial relationships |
| `dashboard / 仪表盘` | Metrics, KPIs |
| `periodic-table / 周期表` | Categorized collections |
| `comic-strip / 漫画叙事` | Narratives, sequences |
| `story-mountain / 故事山` | Plot structure, tension arcs |
| `jigsaw / 拼图互联` | Interconnected parts |
| `venn-diagram / 韦恩图` | Overlapping concepts |
| `winding-roadmap / 蜿蜒路线图` | Journey, milestones |
| `circular-flow / 循环流程` | Cycles, recurring processes |
| `dense-modules / 高密度信息模块` | High-density modules, data-rich guides |
...
| Style | Description |
|-------|-------------|
| `craft-handmade / 手工纸艺` | Hand-drawn, paper craft (default) |
| `claymation / 粘土动画` | 3D clay figures, stop-motion |
| `kawaii / 日系可爱` | Japanese cute, pastels |
| `storybook-watercolor / 绘本水彩` | Soft painted, whimsical |
| `chalkboard / 黑板粉笔` | Chalk on black board |
| `cyberpunk-neon / 赛博霓虹` | Neon glow, futuristic |
| `bold-graphic / 粗犷漫画` | Comic style, halftone |
| `aged-academia / 复古学术` | Vintage science, sepia |
| `corporate-memphis / 孟菲斯商务` | Flat vector, vibrant |
| `technical-schematic / 技术蓝图` | Blueprint, engineering |
| `origami / 折纸几何` | Folded paper, geometric |
| `pixel-art / 像素艺术` | Retro 8-bit |
| `ui-wireframe / UI线框` | Grayscale interface mockup |
| `subway-map / 地铁线路图` | Transit diagram |
| `ikea-manual / 宜家说明书` | Minimal line art |
| `knolling / 整理收纳` | Organized flat-lay |
| `lego-brick / 乐高积木` | Toy brick construction |
| `pop-laboratory / 流行实验室` | Blueprint grid, coordinate markers, lab precision |
| `morandi-journal / 莫兰迪手账` | Hand-drawn doodle, warm Morandi tones |
| `retro-pop-grid / 复古波普网格` | 1970s retro pop art, Swiss grid, thick outlines |
| `hand-drawn-edu / 手绘教育` | Macaron pastels, hand-drawn wobble, stick figures |
| `retro-popup-pop / 复古弹窗波普` | Retro popup collage, vintage UI, thick outlines, flat pop colors |
...
When generating infographics containing Chinese, Japanese, Korean, or any non-Latin text (headings, labels, callouts, data values):

1. **ALL text MUST be rendered with perfect accuracy** — NO garbled characters (乱码), NO missing strokes, NO corrupted glyphs, NO incorrect characters
2. Use ONLY standard, widely-supported typography — avoid decorative or experimental text rendering for CJK characters
3. If the AI image model cannot reliably render Chinese/Asian text:
   - Prefer SVG-based generation over raster image generation for text-heavy infographics
   - Reduce on-image text; convey meaning through icons, visual hierarchy, and layout
   - Keep labels extremely short (2-4 characters max)
4. **NEVER output an infographic with garbled, unreadable, or corrupted text** — if generation produces garbled text, regenerate with SVG approach or simplified text
5. Add `CRITICAL: All text must be clearly legible with zero garbled characters or incorrect text. If text cannot be rendered perfectly, generate as SVG with system fonts.` to EVERY infographic generation prompt
