---
name: "手帐拼贴卡"
description: "结合白边贴纸、和纸胶带与中文手写效果，将照片快速转换为小红书/INS 风格极具质感的手帐拼贴卡。"
version: 1.0.1
---
...
4. **Phase 4 提取与生成图片** → 严格按以下两个子步骤串行：
   - **第一步：提取主体**。必须先调用 `heyclaw_image_segment` 工具（设置 `type="general"`，传入用户上传图片的真实路径）对原图进行背景去除与主体分割，获取返回的分割主体图 URL。
   - **第二步：最终生图**。拿到第一步返回的分割图 URL 后，将该 URL 作为垫图（填入 `image` 字段），配合 Phase 3 编译出的结构化提示词，**优先使用 Seedream（Doubao-Seedream-5.0-lite）模型**调用当前环境可用的文生图工具 `heyclaw_image_generate` 生成最终的手账拼贴卡图片。无 Seedream 时兼容其他图像模型。
