...
description: Analyzes article structure, identifies positions requiring visual aids, generates illustrations with Type × Style × Palette three-dimension approach. Use when user asks to "illustrate article", "文章生成图片", "generate images for article", or "为文章配图".
version: 1.117.4
metadata:
  openclaw:
...
| Type | Best For |
|------|----------|
| `infographic` / 信息图 | Data, metrics, technical / 数据图表、技术内容 |
| `scene` / 场景图 | Narratives, emotional / 叙事、情感表达 |
| `flowchart` / 流程图 | Processes, workflows / 流程、工作流 |
| `comparison` / 对比图 | Side-by-side, options / 并排对比、选项 |
| `framework` / 框架图 | Models, architecture / 模型、架构 |
| `timeline` / 时间线 | History, evolution / 历史、演进 |
...
When generating article illustrations containing Chinese, Japanese, Korean, or any non-Latin text (labels, captions, annotations, titles):
...
3. If the AI image model cannot reliably render Chinese/Asian text:
   - Reduce or eliminate on-image text; convey meaning through visuals, icons, and layout instead
   - Keep any text extremely short (2-4 characters per label max)
   - Use clean sans-serif typography for any essential text
4. **NEVER output an illustration with garbled, unreadable, or corrupted text** — if generation produces garbled text, regenerate with simplified or removed text elements
5. Add `CRITICAL: All text must be clearly legible with zero garbled characters or incorrect text. If text cannot be rendered perfectly, omit it entirely and use visual elements instead.` to EVERY illustration prompt
