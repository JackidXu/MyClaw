...
## Text Anti-Garbling Rules

When constructing prompts for any illustration type that contains text:

1. Add to EVERY prompt: `CRITICAL: All text in the image must be perfectly legible with ZERO garbled characters (乱码), ZERO incorrect characters, ZERO corrupted glyphs.`
2. For Chinese/Japanese/Korean text: specify `Use clean sans-serif fonts only (e.g., PingFang SC, Noto Sans SC style). Keep text labels to 3-5 characters maximum.`
3. If the style involves decorative or artistic text treatment, add: `Text must remain readable despite stylistic treatment. No garbled or corrupted characters.`
4. If the illustration type heavily depends on text (labels, annotations), recommend SVG-based generation over raster image generation as a fallback.
