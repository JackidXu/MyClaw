...
- **TEXT ACCURACY (CRITICAL)**: Every single character in the image MUST be correctly rendered, clearly legible, and match the source content exactly. NO garbled text (乱码), NO hallucinated/phantom characters, NO distorted or unreadable strokes, NO mixed or wrong scripts. This is the highest priority — a beautiful image with broken text is a FAILED image.
...
- **For Chinese content**: Every Chinese character must be accurately rendered with correct strokes and structure. Absolutely NO garbled characters (乱码), NO wrong characters, NO incomplete strokes, NO alien/unreadable symbols. If unsure about a character, use a simpler synonym rather than risking garbled output.
...
- **TEXT ACCURACY (CRITICAL)**: All text must be accurately rendered — NO garbled characters, NO hallucinated text, every character must be legible and correct
...
- [ ] **Text accuracy rules are present** (anti-garbled, anti-hallucination, character correctness)
