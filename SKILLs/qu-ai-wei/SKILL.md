...
name: qu-ai-wei
description: |
  去除简体中文文本里的 AI 写作痕迹，不虚构事实，让终稿干净、精准、像真人写的。
  当用户说「去 AI 味」「去ai味」「去掉 AI 腔」「去除 AI 痕迹」「改得说人话」「写得更像人」「像真人写的」「润色一下让它更自然」「太书面了改口语点」「humanize 这段中文」「这段中文太 AI 了改一下」「帮我改改这段」「读着别扭」「太生硬了」时自动调用。
...
| ——（破折号） | 逗号或句号 | 口播中没有破折号 |
| 不是 X，是 Y | 反问 + 陈述，或直接陈述 | 已由 #48 口播附加条款覆盖 |
| 综上所述 | 所以 | |
| 换言之 | 就是说 | |
| 正如...所说 | 直接引用或"有人说过" | |
...
- **#48 否定对举下定义句** — 包括 `这不是 X，而是 Y` / `不是 X，也不是 Y` / `既不是 X，也不是 Y`。只有"工整对称 + X/Y 是抽象桶词 + 句子不落到具体事实 / 约束 / 动作"才触发；用它剥离误解、落到具体真相时不触发。**口播附加条款**：语体为「内容/自媒体」口播逐字稿时，`不是 X，是 Y` / `不是 X，而是 Y` **无论 X/Y 是否具体均触发**——该句式是书面修辞结构，口播中真人不会用压缩式否定对举，会改用反问句、直接陈述或拆成两句。
