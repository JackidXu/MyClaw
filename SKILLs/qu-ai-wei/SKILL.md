...
description: >
  ...
  当用户说「去 AI 味」「去ai味」「去掉 AI 腔」「去除 AI
  痕迹」「改得说人话」「写得更像人」「像真人写的」「润色一下让它更自然」「太书面了改口语点」「humanize 这段中文」「这段中文太 AI
  了改一下」「帮我改改这段」「读着别扭」「太生硬了」时自动调用。
...
**隐式调用（自然语言触发）：** 当用户的请求包含下列意图时，应主动调用本 skill，无需用户显式键入命令：
...
| 用法 | 行为 |
|---|---|
| 用户贴出文本 + 去 AI 味意图 | 默认对简体中文改写 |
| 用户只说「去 AI 味」无文本 | 询问用户粘贴文本；可选做轻量级语音校准 |
| 自然语言触发 | 见上节「隐式调用」 |
...
| §2 | 语体降级保护 | `避免过度修正` · `语体降级保护` | 改写终稿跟原文语体档最多差一档。跨档要先问用户场景，不许自作主张。 |
| §3 | 过度消毒反制 | `过度消毒反制` · `AI 不敢写测试` | 终稿至少留一处**毛边**（"毛边"指原文里那些 AI 写不出来的痕迹：主观判断 / 具体感受 / 圈层语境），不能改成无菌的中性散文。 |
| §4 | 51 条减法规则 | `51 条 AI 腔模式 · 索引表` | 按第零步语体矩阵激活的子集做减法。白名单（体育 / 通用技术缩写 / 人物昵称 / 护肤成分）是减法的硬例外，见 [references/whitelists.md](references/whitelists.md)。 |
| §5 | 六条打磨抬升 | `主动打磨` | 弱动词 / 机关枪节奏 / filler / 弱化具体细节 / 倒装语序 / 跨档语体 —— 看到就改，不等 AI 腔明显。 |

**仲裁示例：**

- §4 减法要删某词，但删掉这段就没毛边了 → §3 胜出，保留。
- §5 打磨想把"速度提升"改成"快了 3 倍"，但原文没数字 → §1 胜出，放弃。
- §3 想从原文里挖一处毛边，挖出来的句子里其实带着改写者编造的数字 → §1 胜出，这处不算数，另挑或不留。
- 把对外公告改成朋友圈口吻"接地气" → §2 阻止，要么保留原档，要么先问用户场景。
- 用户输入的是繁體或非简中 → 提示"当前只支持简体中文"，不自动转换。
...
> **触发条件**：语体判定为「内容 / 自媒体」**且**文本是口播逐字稿（短视频 / 直播口播稿，不是公众号文章 / 小红书图文）。
>
> 判定信号：文本中出现画面指引（`[画面]` / `[镜头]`）、段落标注时长（`0-4s`）、口播提示词（`旁白` / `口播` / `画外音`）、或用户 / 调用方明确标注为口播脚本。

**为什么需要单独一层**：qu-ai-wei 的 51 条模式按「文章 AI 腔」设计——很多模式的改后样例本身仍然是书面语（「该项政策可能对经济有正向影响」「团队接下来两个季度主攻两件事」），在文章里是干净的好中文，但在口播里依然不自然。AI 生成口播脚本时大量使用这些书面句式，qu-ai-wei 按文章标准检测会漏判——不是模式没触发，而是**改写方向不对**：改成了"更干净的文章"，而非"能直接说出口的话"。

#### 一、口播加固触发（更严格的触发条件）

以下模式在口播逐字稿中**降低触发阈值 / 取消文章例外**：

| 模式 | 文章默认行为 | 口播加固行为 | 原因 |
|---|---|---|---|
| **#10** 不仅...更/还是... | 单次可保留 | **只要出现即改**——拆成两句直接陈述 | 口播不用"不仅X，更Y"这种书面并列结构 |
| **#16** 破折号同位补注 | 书面语体≥3次触发 | **≥1次即改**——破折号在口播中不存在，改为逗号或拆句 | 口播中没有破折号这个标点 |
| **#25** 破折号频率 | 自媒体≥2处触发 | **≥1处即改**——同上 | 同上 |
| **#34** 逻辑连接词 | 短段≥3个才触发 | **"然而/此外/综上所述/由此可见/不仅如此/与此同时/换言之"出现即改** | 这些是纯书面逻辑连接词，口播用"但是/还有/所以/同时/就是说" |
| **#40** "X很Y："冒号起手 | 触发但改后可能仍偏书面 | **口播中直接去掉"X很Y："起手，直接说Y的内容** | 口播不用冒号引出结论 |
| **#41** 抽象名词做主语 | 触发但改后可能仍偏书面 | **口播中主语必须具体化**——"工程上的现实"→"真做起来" | 口播主语要听得见画面 |
| **#48** 否定对举 | 已有口播+社交内容附加条款 | ✅ 已处理（覆盖口播逐字稿+朋友圈/社交内容+公众号+小红书等所有自媒体形态） | |

#### 二、口播改写方向（改后终稿必须是口语，不是"更干净的文章"）

即使触发了模式、按文章标准改了，口播终稿还必须过这三条：

1. **能说出来的才留** — 改后如果需要"读"才能理解，说明还是书面语。口播是"听"的，每个词必须在 3 秒内能听懂。改后自己默念一遍，卡壳的地方就是还没改到位。
2. **书面结构→口语结构** — 冒号起手改陈述句、破折号改逗号或句号、逻辑连词换口语连接（"然而"→"但是"、"此外"→"还有"、"综上"→"所以"）、抽象主语换具体主语。
3. **压缩概念要展开** — 文章里"是身份"读者能脑补，口播里听众听到"身份"会愣，必须展开为"有人冒充你的身份"。判断标准：听众听到这个词**会不会愣一下**？会愣就展开。

#### 三、口播不自然表达补充清单

以下表达在文章中合法，但在口播逐字稿中不自然。AI 生成脚本时经常使用，遇到时替换为口语表达。**这些不是新的 AI 腔模式——它们在文章语体里不触发，只在口播逐字稿中触发。**

| 书面表达 | 口播替换 | 说明 |
|---|---|---|
| 值得注意的是 | 直接说后面的内容 | 口播不用预告式 |
| 不可否认 / 毋庸置疑 | 直接说观点 | 同上 |
| 不仅...而且... | 拆成两句直接说 | 口播不用这种并列结构 |
| 基于此 | 所以 | |
| 由此可见 | 所以 | |
| 与此同时 | 同时 / 还有 | |
| 从某种意义上说 | 说白了 / 直接说 | |
| 在很大程度上 | 基本上 / 直接说 | |
| X 很 Y：（冒号起手） | 直接说 Y 的内容 | 口播不用冒号引出 |
| ——（破折号） | 逗号或句号 | 口播中没有破折号 |
| 不是 X，是 Y | 反问 + 陈述，或直接陈述 | 已由 #48 口播+社交内容附加条款覆盖 |
| 综上所述 | 所以 | |
| 换言之 | 就是说 | |
| 正如...所说 | 直接引用或"有人说过" | |
| 在...的背景下 | 直接说背景 | 口播不用这种介词框架 |
| 对...而言 | 对...来说 | |
| 就...而言 | 说... | |
| 基于...的考虑 | 想到... / 考虑到... | |
| 从...的角度来看 | 从...来看 / 说句...的 | |

> **跟语体降级保护（§2）的关系**：口播强化层不违反语体降级保护——口播逐字稿的语体档位本来就是"自媒体"（第二口语档），改写方向是**同档内部**从书面倾向调到口语倾向，不是跨档降级。口播改写后仍是自媒体语体，不是降到"微信聊天"档。
...
0. **先识别语体** — 见上一节，确定是 社交 / 自媒体 / 商务 / 书面 / 特稿 / 品牌广告 / 学术 / 公文 / 高考作文 中的哪一种。这决定后面激活哪些规则。**若判定为「内容/自媒体」且文本是口播逐字稿，额外激活「口播逐字稿强化层」。**
1. **识别 AI 模式** — 扫描下文列出的 51 类模式，**只激活当前语体对应的子集**；若口播强化层激活，同时应用其加固触发和补充清单
2. **改写问题段落** — 把 AI 腔替换成自然表达。**口播逐字稿改写终稿必须是"能直接说出口的话"，不是"更干净的文章"——见口播强化层第二节**
3. **保留原意** — 核心信息不丢
4. **匹配原有语域** — 正式 / 口语 / 技术 / 文艺 / 学术 / 公文 —— **不要统一往日常白话拉平**
5. **注入"魂"** — 不只是去除坏模式，还要让文字有人味
6. **最终 AI 腔体检** — 问自己 "这段文字还有哪里像 AI 写的？"；也问 "**改过头了没？有没有把合规表达错改成 AI 腔？**"。**口播逐字稿额外问："这段话念出来听着自然吗？有没有需要读才能理解的书面表达？"** 简短列出两类残留痕迹，然后再改一遍
...
3. **语体验证** — 改写后跟原文档位一致吗？把学术严谨改成日常白话、公文庄重改成朋友圈调侃，都是降格失败（见冲突仲裁顺序 §2）。
...
4. **最后**，如果以上都做不到，宁可让终稿有轻微"中性散文"味 —— 这比编造事实好。"过度消毒反制"（§3）不是**授权改写者虚构**毛边，见冲突仲裁顺序 §1 vs §3 的反向示例。
...
跟科技圈"英文原样保留"同源，中文写作里还有四类已沉淀的词形，翻成"标准译法"反而是 AI 腔。**冲突仲裁顺序 §4：** 本白名单是减法规则（#17 / #42 / #9）的硬例外。
...
**没有样本时的默认语域：书面 / 一般（中性书面）** — 不装饰、不宣传、不文艺，不主动口语降格。如果用户明确要口语化，再下调到日常中性白话。
...
> **本节归口：** 见冲突仲裁顺序 §3（过度消毒反制）。当 §4 的减法规则会删掉最后一处毛边时 —— 放弃减法，保留毛边。
...
**换句话说：打磨不是"发现原文隐藏的灵感"，而是"把原文已经有的东西弄得更锋利"。** 本节归口冲突仲裁顺序 §5，受 §1（不发明事实）约束。遇到原文毛坯太糙、打磨不出锋利度时 —— 停手、报告、不自造。
...
- **#10 "不仅...更..."空洞并列** — 跟 #48（`这不是 X，而是 Y / 不是 X，也不是 Y`）联动，都属 AI 偏爱的铺陈 / 定义结构。**口播加固**：只要出现即改。
...
- **#16 长破折号解释句**（语体 + 密度双触发）— `X——也就是 Y` 同位补注。跟 #25 分工。**口播加固**：≥1 次即改。
...
- **#34 逻辑连接词滥用 / 空转** — "然而 / 此外 / 因此 / 综上所述 / 换言之"短段三个以上。学术语体里**空转的**仍要改。**口播加固**："然而/此外/综上所述/由此可见/与此同时/换言之"出现即改。
...
- **#25 破折号滥用**（频率触发）— 跟 #16 分工。日常 / 自媒体 / 商务 / 邮件 500 字内 ≥2 处不承担功能的 `——` 即触发。品牌广告语体允许节奏设计。**口播加固**：≥1 处即改。
...
- **#40 "X 很 Y：" 形容词+冒号起手式** — "逻辑很清晰： / 结论很明确："。英文 `The logic is clear：` 翻译腔。**口播加固**：直接去掉起手。
- **#41 抽象名词做主语 + 形容词收尾** — "工程上的现实比这些数字难看"。英文 `The reality of X is uglier than Y` 直译骨架。**口播加固**：主语必须具体化。
- **#42 有中文译法却留着的英文词** — `argument / logic / workflow / feature` 等。**硬白名单例外（MCP / API / TLDR / PUA / city walk / embedding / token 等）见「中英混排」章节（冲突仲裁顺序 §4 的硬例外）**。
...
- **#48 否定对举下定义句** — 包括 `这不是 X，而是 Y` / `不是 X，也不是 Y` / `既不是 X，也不是 Y`。只有"工整对称 + X/Y 是抽象桶词 + 句子不落到具体事实 / 约束 / 动作"才触发；用它剥离误解、落到具体真相时不触发。**口播+社交内容附加条款**：语体为「内容/自媒体」时（含口播逐字稿、朋友圈/社交内容、公众号文章、小红书等所有形态），`不是 X，是 Y` / `不是 X，而是 Y` **无论 X/Y 是否具体均触发**——该句式是书面修辞结构，真人写朋友圈、拍口播、发小红书都不会用压缩式否定对举，会改用反问句、直接陈述或拆成两句。只有书面/一般和叙事非虚构/特稿语体保留默认判定（工整对称 + 抽象桶词 + 不落具体才触发）。**改写方法论**：触发时必须**重组整句**换一种表达方式，理解原句想传达的意思后用完全不同的句式重新表达。**禁止只替换句式中的个别词语来保留原有结构**——例如把"不是没钱，是不敢花"改成"钱有，但不敢花"是正确重组；但改成"不叫没钱，是不敢花"或"不是没钱就完了，是不敢花"则是换词保结构，依然别扭。判定标准：改写后的句子是否还能看出"否定一个东西+肯定另一个东西"的对举骨架？看得出 → 还没改到位，继续重组。
...
3. 改写每处问题段落 —— **减法**（清 AI 腔） **+ 主动打磨**（6 条打磨动作，见「主动打磨」章节）；保留原意、保留输入字形、**禁止虚构**（「事实发明禁令」硬约束持续生效）。**改写原则**：识别到 AI 句式时，必须**重组整句**换一种表达方式——理解原句意思后用完全不同的句式重新表达。**禁止只替换句式中的个别词语来保留原有结构**，那样只是把一种 AI 腔换成另一种别扭的变体（如把"不是X，是Y"改成"不叫X，得Y"依然是对举骨架）。判定标准：改写后的句子是否还能看出原句式的骨架？看得出 → 还没改到位，继续重组。
4. 确认改写后（整篇 打磨自检 · 五问）：
   - **用词精准**（每个词都必须，无损可删的切掉）
   - **节奏变化**（朗读无机关枪段）
...
- `语体判断`（识别为 社交 / 自媒体 / 商务 / 书面 / 特稿 / 品牌广告 / 学术 / 公文 / 高考作文 中的哪一种）
- `初稿改写`
- `自审`：
  - `AI 不敢写测试`（底线 · ≥1 句）
  - `整篇 打磨自检`五问（合格线 · 都要过）
