...
> ⚠️ **口播附加条款：** 口播逐字稿中"不仅...更/还是..."**只要出现即改**——口播不用这种书面并列结构。改写方向：拆成两句直接陈述，或用"再说了""还有"等口语连接词。见 SKILL.md「口播逐字稿强化层」。
...
> - **学术 / 公文 / 法律 / 高考应试作文**: 不触发。
>
> ⚠️ **口播附加条款：** 口播逐字稿中**≥1 次即触发**——破折号在口播中不存在，所有 `——` 改为逗号、句号或拆句。口播中没有"同位补注"这种修辞结构。见 SKILL.md「口播逐字稿强化层」。
>
...
> ⚠️ **口播附加条款：** 口播逐字稿中 `然而 / 此外 / 综上所述 / 由此可见 / 不仅如此 / 与此同时 / 换言之` **出现即改**——这些是纯书面逻辑连接词，口播中完全不会用。替换表：然而→但是 / 此外→还有 / 综上所述→所以 / 由此可见→所以 / 与此同时→同时 / 换言之→就是说。见 SKILL.md「口播逐字稿强化层」。
...
> ⚠️ **口播附加条款：** 口播逐字稿中**≥1 处即触发**——破折号在口播中不存在，所有 `——` 改为逗号或句号。见 SKILL.md「口播逐字稿强化层」。
...
> ⚠️ **口播附加条款：** 口播逐字稿中**直接去掉"X 很 Y："起手**——口播不用冒号引出结论，直接说后面的内容。见 SKILL.md「口播逐字稿强化层」。
...
> ⚠️ **口播附加条款：** 口播逐字稿中**主语必须具体化**——口播是"听"的，抽象主语（"工程上的现实""市场的成熟度"）听众无法在 3 秒内消化。改为主语带具体动作或场景（"真做起来""市场还没到那一步"）。见 SKILL.md「口播逐字稿强化层」。
...
**⚠ 口播语体附加条款（重要）：** 当语体为「内容/自媒体」中的口播逐字稿时，`不是 X，是 Y` / `不是 X，而是 Y` 句式**无论 X/Y 是否具体均触发**。原因：该句式是书面修辞结构（否定 + 对举 + 压缩定义），口播中真人不会用这种压缩式否定对举——会改用反问句、直接陈述或拆成两句。即使 X/Y 是具体词（如"市场""身份"），口播中也不会说"最大的风险不是市场，是身份"，而会说"你以为盯盘就安全了？有人冒充你的身份，这才是你该慌的"。

改写策略（口播场景）：
- 拆成反问 + 直接陈述：「你以为风险在 X？真正该慌的是 Y。」
- 只说结论，删掉否定对举：「这才是你该慌的——Y。」
- 用"压根"等副词替代"不是"结构：「风险压根就不在 X 那边，在 Y。」（仅在"压根不在X，在Y"的口语自然度成立时用）
