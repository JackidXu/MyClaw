...
CRITICAL: All text elements (title, subtitle, tags) must be rendered with PERFECT accuracy — NO garbled characters (乱码), NO incorrect characters, NO missing strokes. If the model cannot render text perfectly, simplify to title-only or text-none mode. For CJK text, use clean sans-serif typography only. ALWAYS append this to every cover generation prompt: "ALL text must be perfectly legible — zero garbled characters, zero incorrect text. If text cannot be rendered perfectly, use simplified text or omit it entirely."
