---
name: cover-image
description: Generates article cover images with 5 dimensions (type, palette, rendering, text, mood) combining 11 color palettes and 7 rendering styles. Supports cinematic (2.35:1), widescreen (16:9), and square (1:1) aspects. Use when user asks to "generate cover image", "create article cover", "创建封面图", "生成封面图", or "make cover".
version: 1.117.5
metadata:
  openclaw:
...
When generating cover images containing Chinese, Japanese, Korean, or any non-Latin text (titles, subtitles, tags):

1. **ALL text MUST be rendered with perfect accuracy** — NO garbled characters (乱码), NO missing strokes, NO corrupted glyphs, NO incorrect/mixed-up characters
...
4. **NEVER output an image with garbled, unreadable, or corrupted text** — if generation produces garbled text, regenerate with simplified or removed text elements
5. Add `CRITICAL: All text must be clearly legible with zero garbled characters or incorrect text. If text cannot be rendered perfectly, omit it entirely and use visual elements instead.` to EVERY cover image generation prompt
