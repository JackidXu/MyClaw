---
name: diagram
description: Create professional, dark-themed SVG diagrams of any type — architecture diagrams, flowcharts, sequence diagrams, structural diagrams, mind maps, timelines, illustrative/conceptual diagrams, and more. Use this skill whenever the user asks for any kind of technical or conceptual diagram, visualization of a system, process flow, data flow, component relationship, network topology, decision tree, org chart, state machine, or any visual representation of structure/logic/process. Also trigger when the user says "画个图表" "画一个架构图" "diagram" "flowchart" "sequence diagram" "制作图表" or uploads content and asks to visualize it. Output is always a standalone .svg file.
version: 1.117.3
---
...
|------|-------------|-------------------|
| **Architecture / 架构图** | System components & relationships | Grouped boxes, connection arrows, region boundaries |
| **Flowchart / 流程图** | Decision logic, process steps | Diamond decisions, rounded step boxes, directional flow |
| **Sequence / 时序图** | Time-ordered interactions between actors | Vertical lifelines, horizontal messages, activation bars |
| **Structural / 结构图** | Class diagrams, ER diagrams, org charts | Compartmented boxes, typed relationships (inheritance, composition) |
| **Mind Map / 思维导图** | Brainstorming, topic exploration | Central node, radiating branches, organic layout |
| **Timeline / 时间线** | Chronological events | Horizontal/vertical axis, event markers, period spans |
| **Illustrative / 概念插画** | Conceptual explanations, comparisons | Free-form layout, icons, annotations, visual metaphors |
| **State Machine / 状态机** | State transitions, lifecycle | Rounded state nodes, labeled transitions, start/end markers |
| **Data Flow / 数据流图** | Data transformation pipelines | Process bubbles, data stores, external entities |
...
6. **Chinese text support:** **TEXT ACCURACY — CRITICAL**: All Chinese text in SVG diagrams must use system fonts (PingFang SC, Noto Sans SC, Microsoft YaHei) to ensure PERFECT rendering with zero garbled characters. Never rely on AI image models for text rendering in diagrams — SVG text elements are rendered by the browser/OS font system and are always accurate. When labels contain Chinese characters, use `font-family: 'JetBrains Mono', 'Noto Sans SC', 'PingFang SC', sans-serif'` and increase box widths — CJK characters are wider
